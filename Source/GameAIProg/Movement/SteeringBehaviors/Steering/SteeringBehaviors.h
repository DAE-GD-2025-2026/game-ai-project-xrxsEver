...
public:
	...
	virtual SteeringOutput CalculateSteering(float DeltaT, ASteeringAgent &Agent) override;
	void SetTargetRadius(float X);
	float GetOriginalMaxSpeed() const { return m_OriginalMaxSpeed; }
...
public:
	...
	void SetMaxWanderAngle(float rad) { m_MaxWanderAngle = rad; }

protected:
	float m_OffsetDistance = 50.f;
	float m_Radius = 45.f;
	float m_MaxAngleChange = 0.035f;
	float m_MaxWanderAngle = FMath::DegreesToRadians(60.f);
